...
// ── CRUD endpoints (availability & compliance) ──
MapCrud<IsoClassRating>("/api/iso-class-ratings", db => db.IsoClassRatings);
MapCrud<UptimeRecord>("/api/uptime-records", db => db.UptimeRecords);
MapCrud<ComplianceChecklist>("/api/compliance-checklists", db => db.ComplianceChecklists);
MapCrud<EvidenceDocument>("/api/evidence-documents", db => db.EvidenceDocuments);
MapCrud<DcIncident>("/api/dc-incidents", db => db.DcIncidents);
...
app.MapPost("/api/seed", async (AppDbContext db) =>
{
    ...
    db.IsoClassRatings.RemoveRange(db.IsoClassRatings);
    db.UptimeRecords.RemoveRange(db.UptimeRecords);
    db.ComplianceChecklists.RemoveRange(db.ComplianceChecklists);
    db.EvidenceDocuments.RemoveRange(db.EvidenceDocuments);
    db.DcIncidents.RemoveRange(db.DcIncidents);
    await db.SaveChangesAsync();
    ...
    // ── Availability & Compliance seed ──
    db.IsoClassRatings.AddRange(
        new IsoClassRating { Site = "DC-EAST-01", System = "Power Distribution", ClassRating = 3, Justification = "Dual power paths active simultaneously; scheduled maintenance possible without IT load impact", AssessedBy = "J. Martinez", AssessedDate = "2026-01-15", Status = "active" },
        new IsoClassRating { Site = "DC-EAST-01", System = "Cooling Plant", ClassRating = 3, Justification = "N+1 CRAC units with redundant chiller loops; concurrent maintainability verified during 2025 audit", AssessedBy = "J. Martinez", AssessedDate = "2026-01-15", Status = "active" },
        new IsoClassRating { Site = "DC-EAST-01", System = "Network Infrastructure", ClassRating = 4, Justification = "Fully redundant spine-leaf topology with no single point of failure; active-active uplinks confirmed", AssessedBy = "L. Chen", AssessedDate = "2026-01-20", Status = "active" },
        new IsoClassRating { Site = "DC-WEST-02", System = "Power Distribution", ClassRating = 2, Justification = "Redundant UPS with manual transfer switch; maintenance requires brief scheduled downtime window", AssessedBy = "A. Patel", AssessedDate = "2026-02-03", Status = "active" },
        new IsoClassRating { Site = "DC-WEST-02", System = "Cooling Plant", ClassRating = 2, Justification = "Single chiller with redundant CRACs; chiller maintenance requires temporary capacity reduction", AssessedBy = "A. Patel", AssessedDate = "2026-02-03", Status = "active" },
        new IsoClassRating { Site = "DC-EU-03", System = "Power Distribution", ClassRating = 4, Justification = "Fault-tolerant design with 2N redundancy; continuous operation maintained during any single component failure", AssessedBy = "M. Weber", AssessedDate = "2026-01-28", Status = "active" },
        new IsoClassRating { Site = "DC-EU-03", System = "Physical Security", ClassRating = 3, Justification = "Dual-factor access at all entry points; CCTV with 99-day retention; mantrap at server hall entrance", AssessedBy = "M. Weber", AssessedDate = "2026-01-28", Status = "active" },
        new IsoClassRating { Site = "DC-EAST-01", System = "Physical Security", ClassRating = 2, Justification = "Badge access and CCTV in place; some secondary access points lack biometric verification", AssessedBy = "S. Thompson", AssessedDate = "2026-02-10", Status = "review" }
    );

    db.UptimeRecords.AddRange(
        new UptimeRecord { Site = "DC-EAST-01", System = "Power Distribution", UptimeMinutes = 43754, DowntimeMinutes = 46, PeriodStart = "2026-01-01", PeriodEnd = "2026-01-31", SlaTarget = 99.9, Incidents = 1, Notes = "Brief UPS transfer on 2026-01-14 during maintenance window" },
        new UptimeRecord { Site = "DC-EAST-01", System = "Cooling Plant", UptimeMinutes = 43800, DowntimeMinutes = 0, PeriodStart = "2026-01-01", PeriodEnd = "2026-01-31", SlaTarget = 99.9, Incidents = 0, Notes = "" },
        new UptimeRecord { Site = "DC-EAST-01", System = "Network Infrastructure", UptimeMinutes = 43800, DowntimeMinutes = 0, PeriodStart = "2026-01-01", PeriodEnd = "2026-01-31", SlaTarget = 99.99, Incidents = 0, Notes = "" },
        new UptimeRecord { Site = "DC-WEST-02", System = "Power Distribution", UptimeMinutes = 43622, DowntimeMinutes = 178, PeriodStart = "2026-01-01", PeriodEnd = "2026-01-31", SlaTarget = 99.9, Incidents = 2, Notes = "Generator test failure on 2026-01-09 caused 2h 58m outage" },
        new UptimeRecord { Site = "DC-WEST-02", System = "Cooling Plant", UptimeMinutes = 43765, DowntimeMinutes = 35, PeriodStart = "2026-01-01", PeriodEnd = "2026-01-31", SlaTarget = 99.9, Incidents = 1, Notes = "Chiller control board fault; failover to secondary took 35 min" },
        new UptimeRecord { Site = "DC-EU-03", System = "Power Distribution", UptimeMinutes = 43800, DowntimeMinutes = 0, PeriodStart = "2026-01-01", PeriodEnd = "2026-01-31", SlaTarget = 99.99, Incidents = 0, Notes = "No incidents — Class 4 operation confirmed" },
        new UptimeRecord { Site = "DC-EAST-01", System = "Power Distribution", UptimeMinutes = 40314, DowntimeMinutes = 6, PeriodStart = "2026-02-01", PeriodEnd = "2026-02-28", SlaTarget = 99.9, Incidents = 0, Notes = "" },
        new UptimeRecord { Site = "DC-WEST-02", System = "Power Distribution", UptimeMinutes = 40218, DowntimeMinutes = 102, PeriodStart = "2026-02-01", PeriodEnd = "2026-02-28", SlaTarget = 99.9, Incidents = 1, Notes = "Planned PDU swap overran maintenance window by 102 min" }
    );

    db.ComplianceChecklists.AddRange(
        new ComplianceChecklist { Category = "Power", Requirement = "Dual independent power feeds", IsoReference = "ISO 22237-3:2018 §5.2", Description = "Two physically separate utility power feeds from independent substations", Status = "compliant", AssignedTo = "J. Martinez", DueDate = "2026-03-31", Evidence = "DOC-PWR-001", Notes = "Verified during Q4 2025 infrastructure audit" },
        new ComplianceChecklist { Category = "Power", Requirement = "UPS runtime ≥ 10 minutes at full load", IsoReference = "ISO 22237-3:2018 §5.4", Description = "All UPS systems must provide minimum 10-minute backup at 100% rated load", Status = "compliant", AssignedTo = "J. Martinez", DueDate = "2026-03-31", Evidence = "DOC-UPS-003", Notes = "UPS load test completed 2026-01-10; all units pass" },
        new ComplianceChecklist { Category = "Power", Requirement = "Generator auto-start within 15 seconds", IsoReference = "ISO 22237-3:2018 §5.5", Description = "Standby generators must auto-start and reach rated voltage/frequency within 15 seconds of mains failure", Status = "non-compliant", AssignedTo = "J. Martinez", DueDate = "2026-04-15", Evidence = "", Notes = "DC-WEST-02 generator recorded 22s start time on 2026-01-09 test. Remediation in progress." },
        new ComplianceChecklist { Category = "Cooling", Requirement = "N+1 cooling redundancy", IsoReference = "ISO 22237-4:2018 §5.3", Description = "Minimum N+1 cooling unit redundancy across all server halls", Status = "compliant", AssignedTo = "A. Patel", DueDate = "2026-06-30", Evidence = "DOC-COOL-002", Notes = "" },
        new ComplianceChecklist { Category = "Cooling", Requirement = "Inlet temperature ≤ 27°C (ASHRAE A1)", IsoReference = "ISO 22237-4:2018 §5.6", Description = "Server inlet air temperature must not exceed 27°C at any rack face", Status = "compliant", AssignedTo = "A. Patel", DueDate = "2026-06-30", Evidence = "DOC-COOL-005", Notes = "Continuous monitoring active; max recorded 24.8°C" },
        new ComplianceChecklist { Category = "Physical Security", Requirement = "Mantrap at all primary entry points", IsoReference = "ISO 22237-5:2018 §5.2", Description = "Physical mantrap (double-door airlock) required at all server hall entry points", Status = "in-progress", AssignedTo = "S. Thompson", DueDate = "2026-05-01", Evidence = "", Notes = "DC-EAST-01 secondary access point installation scheduled Q2 2026" },
        new ComplianceChecklist { Category = "Physical Security", Requirement = "CCTV coverage of all aisles", IsoReference = "ISO 22237-5:2018 §5.4", Description = "Continuous CCTV coverage of all hot/cold aisles with minimum 90-day retention", Status = "compliant", AssignedTo = "S. Thompson", DueDate = "2026-06-30", Evidence = "DOC-SEC-004", Notes = "99-day retention confirmed; cameras cover 100% of floor area" },
        new ComplianceChecklist { Category = "Network", Requirement = "Redundant uplinks to all racks", IsoReference = "ISO 22237-6:2018 §5.3", Description = "Each rack must have at least two independent network paths to core switches", Status = "compliant", AssignedTo = "L. Chen", DueDate = "2026-06-30", Evidence = "DOC-NET-001", Notes = "" },
        new ComplianceChecklist { Category = "Environmental", Requirement = "24/7 environmental monitoring", IsoReference = "ISO 22237-4:2018 §5.8", Description = "Continuous monitoring of temperature, humidity, and particulates with alerting", Status = "compliant", AssignedTo = "A. Patel", DueDate = "2026-06-30", Evidence = "DOC-ENV-002", Notes = "BMS integration active; alerts configured for all thresholds" },
        new ComplianceChecklist { Category = "General", Requirement = "Annual ISO 22237 internal audit", IsoReference = "ISO 22237-1:2018 §6.2", Description = "Formal internal audit of all ISO 22237 requirements at least annually", Status = "compliant", AssignedTo = "M. Weber", DueDate = "2026-12-31", Evidence = "DOC-AUD-2025", Notes = "2025 audit completed Nov 2025; 2026 audit scheduled Nov 2026" },
        new ComplianceChecklist { Category = "General", Requirement = "Business continuity plan documented", IsoReference = "ISO 22237-1:2018 §6.4", Description = "Documented and tested BCP covering all critical systems and recovery procedures", Status = "in-progress", AssignedTo = "M. Weber", DueDate = "2026-06-01", Evidence = "", Notes = "BCP draft under review; tabletop exercise planned Q2 2026" },
        new ComplianceChecklist { Category = "Power", Requirement = "EPO system in all server halls", IsoReference = "ISO 22237-3:2018 §5.8", Description = "Emergency power-off system accessible from hall entry and remote stations", Status = "pending", AssignedTo = "", DueDate = "", Evidence = "", Notes = "Assessment not yet started" }
    );

    db.EvidenceDocuments.AddRange(
        new EvidenceDocument { Title = "Annual Infrastructure Audit Report 2025", Type = "Audit Report", IsoReference = "ISO 22237-1:2018 §6.2", Description = "Full internal audit of all DC infrastructure against ISO 22237 requirements", FileName = "infra-audit-2025-final.pdf", FileSize = "4.2 MB", UploadedBy = "M. Weber", UploadedDate = "2025-11-28", ExpiryDate = "2026-11-28", Status = "current", Tags = "audit,annual,iso22237" },
        new EvidenceDocument { Title = "UPS Load Test Results — All Sites", Type = "Test Certificate", IsoReference = "ISO 22237-3:2018 §5.4", Description = "Battery runtime and transfer test results for all UPS units at 100% load", FileName = "ups-load-test-jan2026.pdf", FileSize = "1.8 MB", UploadedBy = "J. Martinez", UploadedDate = "2026-01-12", ExpiryDate = "2027-01-12", Status = "current", Tags = "ups,power,test" },
        new EvidenceDocument { Title = "Power Distribution Design Drawings", Type = "Policy", IsoReference = "ISO 22237-3:2018 §5.2", Description = "As-built drawings showing dual feed paths for all server halls", FileName = "power-drawings-v3.pdf", FileSize = "12.5 MB", UploadedBy = "J. Martinez", UploadedDate = "2025-06-01", ExpiryDate = "", Status = "current", Tags = "power,drawings,design" },
        new EvidenceDocument { Title = "CCTV Coverage Map and Retention Policy", Type = "Policy", IsoReference = "ISO 22237-5:2018 §5.4", Description = "Camera placement diagram and data retention schedule (99 days)", FileName = "cctv-policy-2025.pdf", FileSize = "2.1 MB", UploadedBy = "S. Thompson", UploadedDate = "2025-09-15", ExpiryDate = "2026-09-15", Status = "current", Tags = "security,cctv,policy" },
        new EvidenceDocument { Title = "Cooling System N+1 Verification", Type = "Inspection Record", IsoReference = "ISO 22237-4:2018 §5.3", Description = "On-site inspection confirming N+1 cooling configuration across all halls", FileName = "cooling-n1-inspection.pdf", FileSize = "0.9 MB", UploadedBy = "A. Patel", UploadedDate = "2025-12-10", ExpiryDate = "2026-12-10", Status = "current", Tags = "cooling,redundancy,inspection" },
        new EvidenceDocument { Title = "Network Redundancy Test Report", Type = "Test Certificate", IsoReference = "ISO 22237-6:2018 §5.3", Description = "Failover test results for all network uplinks and spine-leaf paths", FileName = "network-redundancy-test.pdf", FileSize = "1.4 MB", UploadedBy = "L. Chen", UploadedDate = "2025-11-05", ExpiryDate = "2026-11-05", Status = "current", Tags = "network,redundancy,test" },
        new EvidenceDocument { Title = "DC-WEST-02 Risk Assessment", Type = "Risk Assessment", IsoReference = "ISO 22237-1:2018 §6.3", Description = "Site-specific risk assessment identifying generator start-time gap and mitigation plan", FileName = "west02-risk-assessment-2026.pdf", FileSize = "3.2 MB", UploadedBy = "A. Patel", UploadedDate = "2026-01-20", ExpiryDate = "2027-01-20", Status = "current", Tags = "risk,generator,dc-west" },
        new EvidenceDocument { Title = "Staff Security Training Records 2025", Type = "Training Record", IsoReference = "ISO 22237-5:2018 §6.1", Description = "Completion records for mandatory physical and cyber security training", FileName = "security-training-2025.xlsx", FileSize = "0.4 MB", UploadedBy = "S. Thompson", UploadedDate = "2025-12-20", ExpiryDate = "2026-12-20", Status = "current", Tags = "training,security,staff" },
        new EvidenceDocument { Title = "Business Continuity Plan v1 Draft", Type = "Procedure", IsoReference = "ISO 22237-1:2018 §6.4", Description = "Draft BCP covering power, cooling, network, and security failure scenarios", FileName = "bcp-draft-v1.docx", FileSize = "1.1 MB", UploadedBy = "M. Weber", UploadedDate = "2026-02-14", ExpiryDate = "", Status = "under-review", Tags = "bcp,continuity,draft" },
        new EvidenceDocument { Title = "Environmental Sensor Calibration Certificates 2024", Type = "Test Certificate", IsoReference = "ISO 22237-4:2018 §5.8", Description = "Calibration certificates for all temperature and humidity sensors — expired batch", FileName = "sensor-calibration-2024.pdf", FileSize = "2.7 MB", UploadedBy = "A. Patel", UploadedDate = "2024-12-01", ExpiryDate = "2025-12-01", Status = "expired", Tags = "sensors,calibration,environmental" }
    );

    db.DcIncidents.AddRange(
        new DcIncident { Title = "DC-WEST-02 Generator Auto-Start Failure", Category = "Power Failure", Severity = "high", Status = "resolved", AffectedSystem = "Standby Generator G1", Site = "DC-WEST-02", DetectedAt = "2026-01-09T14:22:00", ResolvedAt = "2026-01-09T17:20:00", ReportedBy = "NOC Operator", AssignedTo = "J. Martinez", RootCause = "Faulty fuel solenoid valve prevented engine from reaching rated speed within the 15-second window", Impact = "178 minutes of power supplied by UPS only; no IT load impact but SLA breach recorded", Resolution = "Solenoid valve replaced; generator test passed on 2026-01-11", DowntimeMinutes = 178, IsoClass = "3" },
        new DcIncident { Title = "Chiller Control Board Fault — DC-WEST-02", Category = "Cooling Failure", Severity = "medium", Status = "resolved", AffectedSystem = "Chiller CH-1", Site = "DC-WEST-02", DetectedAt = "2026-01-22T03:15:00", ResolvedAt = "2026-01-22T03:50:00", ReportedBy = "BMS Alert", AssignedTo = "A. Patel", RootCause = "Intermittent fault on chiller control board caused automatic shutdown; failover to secondary chiller", Impact = "35 minutes on secondary chiller; inlet temperatures rose 2.3°C but stayed within ASHRAE limits", Resolution = "Control board replaced under warranty; primary chiller returned to service", DowntimeMinutes = 35, IsoClass = "2" },
        new DcIncident { Title = "UPS Module Replacement — DC-EAST-01", Category = "Power Failure", Severity = "low", Status = "closed", AffectedSystem = "UPS-HALL-B", Site = "DC-EAST-01", DetectedAt = "2026-01-14T02:00:00", ResolvedAt = "2026-01-14T02:46:00", ReportedBy = "Planned Maintenance", AssignedTo = "J. Martinez", RootCause = "Scheduled replacement of end-of-life UPS battery module; transfer to bypass during swap", Impact = "46 minutes on static bypass; redundant feed maintained throughout", Resolution = "Battery module replaced; UPS returned to normal operation", DowntimeMinutes = 46, IsoClass = "3" },
        new DcIncident { Title = "Network Switch Firmware Upgrade Incident", Category = "Network Outage", Severity = "medium", Status = "closed", AffectedSystem = "Core Switch SW-CORE-01", Site = "DC-EAST-01", DetectedAt = "2026-02-05T22:10:00", ResolvedAt = "2026-02-05T22:48:00", ReportedBy = "Network Team", AssignedTo = "L. Chen", RootCause = "Firmware upgrade caused unexpected VLAN configuration reset; redundant path took over but east-west traffic degraded", Impact = "38 minutes of degraded east-west performance; no external connectivity loss", Resolution = "VLAN config restored from backup; upgrade procedure updated to include config backup validation step", DowntimeMinutes = 38, IsoClass = "" },
        new DcIncident { Title = "PDU Overrun During Planned Maintenance — DC-WEST-02", Category = "Power Failure", Severity = "medium", Status = "closed", AffectedSystem = "PDU-ROW-C-02", Site = "DC-WEST-02", DetectedAt = "2026-02-18T01:00:00", ResolvedAt = "2026-02-18T02:42:00", ReportedBy = "Change Management", AssignedTo = "J. Martinez", RootCause = "PDU replacement took longer than planned change window due to unexpected cable labelling mismatch", Impact = "102-minute maintenance overrun; Row C servers on alternate feed; SLA breach recorded", Resolution = "Completed successfully; cable labelling audit initiated for all rows", DowntimeMinutes = 102, IsoClass = "2" },
        new DcIncident { Title = "Humidity Sensor False Alert — DC-EU-03", Category = "Environmental", Severity = "low", Status = "closed", AffectedSystem = "Humidity Sensor H-07", Site = "DC-EU-03", DetectedAt = "2026-02-12T09:45:00", ResolvedAt = "2026-02-12T11:00:00", ReportedBy = "BMS Alert", AssignedTo = "A. Patel", RootCause = "Sensor calibration drift caused false high-humidity alert; actual humidity was within normal range", Impact = "75 minutes of false alert noise; no physical impact", Resolution = "Sensor recalibrated; recalibration schedule updated to quarterly", DowntimeMinutes = 0, IsoClass = "" },
        new DcIncident { Title = "Unauthorised Access Attempt — DC-EAST-01", Category = "Physical Security", Severity = "high", Status = "investigating", AffectedSystem = "Server Hall B Entry", Site = "DC-EAST-01", DetectedAt = "2026-03-02T14:32:00", ResolvedAt = "", ReportedBy = "CCTV Alert", AssignedTo = "S. Thompson", RootCause = "", Impact = "Tailgating attempt detected at server hall B; access denied by mantrap; no breach", Resolution = "", DowntimeMinutes = 0, IsoClass = "" }
    );
